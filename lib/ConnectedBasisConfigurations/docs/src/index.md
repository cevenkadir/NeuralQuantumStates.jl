...
*The batched connected-basis-configuration kernel for quantum lattice models.*
...
``\hat{H}``, which basis configurations ``\vert s' \rangle`` does ``\hat{H}`` connect it to,
and with what matrix element ``\langle s' \vert \hat{H} \vert s \rangle``? This package answers
it for a whole **batch** at once, returning padded arrays rather than a dictionary per sample —
the Julia counterpart of NetKet's `get_conn_padded`.
...
ConnectedBasisConfigurations.jl is developed in the
...
What `?ConnectedBasisConfigurations` shows at the REPL:
