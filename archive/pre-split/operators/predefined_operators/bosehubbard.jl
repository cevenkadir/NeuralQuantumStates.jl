...
import LatticeSpaceGroups as Lattices
...
function connected_basis_configs(
    ...
    _array_type_params = get_array_type_params(
        T_h_Array, T_h, (size_samples[1], 1, size_samples[2:end]...)
    )
    T_h_Array_w_params = T_h_Array{array_type_params...}
    ...
    _xs = reshape(xs, size_samples[1], :, size_samples[2:end]...) |> _T_h_Array_w_params
    ...
    # get the occupation numbers
    nᵢ = @views xs[src.(nn_idx_pairs), fill(:, batch_dims)...]
    nⱼ = @views xs[dst.(nn_idx_pairs), fill(:, batch_dims)...]
    ...
    maskᵢⱼ = @. (nⱼ > 0) && (nᵢ < n_max)
    wsᵢⱼ = reshape(getindex.(Ref(ws), i, j), (N, fill(1, batch_dims)...))
    Nᵢⱼ = sum(maskᵢⱼ, dims=1)

    # for off-diagonal terms (bⱼ^† bᵢ)
    maskⱼᵢ = @. (nᵢ > 0) && (nⱼ < n_max)
    wsⱼᵢ = reshape(getindex.(Ref(ws), j, i), (N, fill(1, batch_dims)...))
    Nⱼᵢ = sum(maskⱼᵢ, dims=1)
    ...
    configs_array_type_params = get_array_type_params(
        ...
        (size_samples[1], N_max_configs, size_samples[2:end]...)
    )
    configs_T_h_Array_w_params = T_h_Array{configs_array_type_params...}
    configs = repeat(
        _xs;
        inner=(1, N_max_configs, fill(1, batch_dims)...)
    ) |> configs_T_h_Array_w_params
    vals = zeros.(Ref(Union{T,Missing}), 1, N_max_configs, size_samples[2:end]...)

    # set the irrelevant output results to "missing"
    batch_dims_ids = 2:(batch_dims+1)
    ...
    vals[:, 1, fill(:, batch_dims)...] .+=
        0.5 * operator.U * sum(xs .* (xs .- 1); dims=1) .+
        operator.V * sum(wsᵢⱼ .* (nᵢ .* nⱼ); dims=1) .-
        operator.μ * sum(xs; dims=1)
    ...
    filtered_configsᵢⱼ = view.(Ref(configs), Ref(:), filterᵢⱼ)
    @views vals[1, filterᵢⱼ] .+=
    ...
    filtered_configsⱼᵢ = view.(Ref(configs), Ref(:), filterⱼᵢ)
    @views vals[1, filterⱼᵢ] .+=
