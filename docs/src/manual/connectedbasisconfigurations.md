...
# Connected basis configurations

*Provided by `ConnectedBasisConfigurations`, which has [its own
site](https://cevenkadir.github.io/NeuralQuantumStates.jl/ConnectedBasisConfigurations/dev/).*

Variational Monte Carlo spends nearly all its time answering one question, several thousand
times per optimization step: given a basis configuration ``\vert s \rangle`` and a Hamiltonian
...
and `ConnectedBasisConfigurations` answers it for a whole **batch** at once, returning padded
arrays rather than a dictionary per sample — the Julia counterpart of NetKet's
`get_conn_padded`.
...
`ConnectedBasisConfigurations` is registered as a package in its own right and has a complete
site:

- [Connected basis configurations](https://cevenkadir.github.io/NeuralQuantumStates.jl/ConnectedBasisConfigurations/dev/manual/connected_states/)
  — compiling, batch shapes, padding, and reusing buffers with `connected_padded!`
- [Local operators](https://cevenkadir.github.io/NeuralQuantumStates.jl/ConnectedBasisConfigurations/dev/manual/local_operators/)
  — the digit ordering, and why `PAULI_Z` is the wrong constant to build a Hamiltonian from
- [Symmetry sectors](https://cevenkadir.github.io/NeuralQuantumStates.jl/ConnectedBasisConfigurations/dev/manual/symmetry_sectors/)
  — folding onto orbit representatives, characters and norm factors
- [Backends](https://cevenkadir.github.io/NeuralQuantumStates.jl/ConnectedBasisConfigurations/dev/manual/backends/)
  — swapping SymBasis or OperatorAlgebra for another library
- [Exact diagonalization](https://cevenkadir.github.io/NeuralQuantumStates.jl/ConnectedBasisConfigurations/dev/examples/exact_diagonalization/)
- [API reference](https://cevenkadir.github.io/NeuralQuantumStates.jl/ConnectedBasisConfigurations/dev/api/connected/)
